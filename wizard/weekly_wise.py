...
class SaleReportAdvance(models.TransientModel):
    ...
    invoice_status = fields.Selection(
        ...
        string='Estado de la factura', default='no', required=True)
    amount_type = fields.Selection(
        [('total', 'Cantidad total'), ('untax', 'Importe no gravado')],
        string='Cantidad total', default='total')
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range('G2:I3', 'Informe de ventas por hora', head) 
        ...
        for rec in data['times']:
            ...
            sheet.write(row, col, 'Orden', format2)
            sheet.set_column(row, col, 15)
            col += 1
            sheet.write(row, col, 'Fecha', format2)
            sheet.set_column(row, col, 15)
            col += 1
            if data['type'] =='total':
                ...
                sheet.write(row, col, 'Total no gravado', format2)
                sheet.set_column(row, col, 15)
                col += 1
            t_total = 0
