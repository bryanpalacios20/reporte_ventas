...
class SaleReportAdvance(models.TransientModel):
    ...
    category_ids = fields.Many2many('product.category', string="Categorias", required=True)
    from_date = fields.Date(string="Fecha de inicio")
    to_date = fields.Date(string="Fecha final")
    status = fields.Selection(
        [('all', 'Todos'), ('draft', 'Borrador'), ('sent', 'Cotización enviada'), ('sale', 'Orden de venta'), ('done', 'Bloqueado')],
        string='Estado', default='all', reqired=True)
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range('G2:N3', 'Informe de sangría de ventas de productos', head)
        if data['start_date'] and data['end_date']:
            sheet.write('G6', 'Desde:', cell_format)
            sheet.merge_range('H6:I6', data['start_date'], txt)
            sheet.write('L6', 'A:', cell_format)
        ...
        for rec in partner:
            ...
            for cat in categ:
                ...
                sheet.write(row, col, 'Producto', format4)
                sheet.set_column('J:J', 17)
                col += 1
                sheet.write(row, col, 'Cantidad', format4)
